  ...
  return (
    <header className="fixed top-0 left-0 w-full z-50 bg-gradient-to-b from-gray-900 to-transparent py-4 hidden md:flex">
      <nav className="container mx-auto flex items-center justify-between px-6">
        ...
        <ul className="flex space-x-8">
          {[
            ...
            <li key={link.name}>
              <Link
                href={link.href}
                className="text-white hover:text-[rgba(0,161,214,0.9)] transition-all duration-300"
              >
                {link.name}
              </Link>
            </li>
        ...
        <div className="flex space-x-4">
          <Link href="/contact">
            <button className="bg-gradient-to-r from-gray-900 to-black text-white px-6 py-2 rounded-full font-semibold text-sm uppercase tracking-wide transition-all duration-300 hover:shadow-[0_0_15px_rgba(0,161,214,0.6)] hover:bg-[rgba(0,161,214,0.9)]">
              Contact Us
            </button>
          </Link>
        </div>
